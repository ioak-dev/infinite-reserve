...
import { INFINITERESERVE_PREF_ADDEXPENSE_ANOTHER } from '../../constants/SessionStorageConstants';
...
const EditIncome = (props: Props) => {
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem(INFINITERESERVE_PREF_ADDEXPENSE_ANOTHER)) {
      setAddAnother(
        sessionStorage.getItem(INFINITERESERVE_PREF_ADDEXPENSE_ANOTHER) === 'true'
      );
    }
  }, []);
  ...
  const toggleAddAnother = () => {
    sessionStorage.setItem(
      INFINITERESERVE_PREF_ADDEXPENSE_ANOTHER,
      (!addAnother).toString()
    );
