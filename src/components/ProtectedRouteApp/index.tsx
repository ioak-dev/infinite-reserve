...
const ProtectedRouteApp = (props: Props) => {
    ...
    const authenticate = async (type: string, redirect = true) => {
        ...
        const accessToken = getSessionValue(`infinitereserve-access_token`);
        const refreshToken = getSessionValue(`infinitereserve-refresh_token`);
        if (accessToken && refreshToken) {
            httpPost(
                ...
                .then((response: any) => {
                    if (response.status === 200) {
                        ...
                        if (response.data?.access_token) {
                            ...
                            setSessionValue(`infinitereserve-access_token`, newAccessToken);
                        }
                        dispatch(
                            addAuth({
                                isAuth: true,
                ...
                .catch((error: any) => {
                    removeSessionValue(`infinitereserve-access_token`);
                    removeSessionValue(`infinitereserve-refresh_token`);
