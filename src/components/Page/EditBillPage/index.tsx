...
import {
  INFINITERESERVE_PREF_ADDBILL_ANOTHER,
  INFINITERESERVE_PREF_ADDBILL_DATE,
...
const EditBillPage = (props: Props) => {
  ...
  const getEmptyBill = (): ReceiptModel => {
    return {
      ...EMPTY_BILL,
      billDate:
        sessionStorage.getItem(INFINITERESERVE_PREF_ADDBILL_DATE) ||
        EMPTY_BILL.billDate,
    };
  };
  ...
  useEffect(() => {
    if (sessionStorage.getItem(INFINITERESERVE_PREF_ADDBILL_ANOTHER)) {
      setAddAnother(
        sessionStorage.getItem(INFINITERESERVE_PREF_ADDBILL_ANOTHER) === 'true'
      );
    }
  }, []);
